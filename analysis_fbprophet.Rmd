...
## Analysis for Google stock using Uni-Variate Model
```{r}
...
future.df <- make_future_dataframe(m, periods = 60)
tail(future.df)
...
pred_uni <- forecast.google[c("ds", "yhat", "yhat_lower", "yhat_upper")]
tail(pred_uni)
...
# pred_uni$ds <- as.Date(pred_uni$ds, format="%Y-%m-%d")
# 
# ggplot() +
#   #geom_line(data = test_df.tesla, aes(x= Date, y = Close, color = "Actual_Close")) + 
#   geom_line(data = pred_uni, aes(x= ds, y = yhat, color = "Predicted")) + 
#   geom_line(data = pred_uni, aes(x= ds, y = yhat_upper, color = "Upper_Limit")) + 
#   geom_line(data = pred_uni, aes(x= ds, y = yhat_lower, color = "Lower_Limit")) + 
#   
...
```
## Analysis for Tesla stock using Uni-Variate Model
```{r}

latest_version <- readRDS(file = version.file)
file.name <- paste("models/prophet/", "V",latest_version, "_TESLA", ".RSD", sep = "")
print(file.name)
...
# Forecast
forecast.tesla <- predict(m, future.df) 

#Exploring forecast results
pred_uni <- forecast.tesla[c("ds", "yhat", "yhat_lower", "yhat_upper")]
tail(pred_uni)

# Plot the model
dyplot.prophet(m, forecast.tesla)
prophet_plot_components(m, forecast.tesla)
...
latest_version <- readRDS(file = version.file)
file.name <- paste("models/prophet/", "V",latest_version, "_APPLE", ".RSD", sep = "")
print(file.name)
...
# Forecast
forecast.apple <- predict(m, future.df) 

#Exploring forecast results
pred_uni <- forecast.apple[c("ds", "yhat", "yhat_lower", "yhat_upper")]
tail(pred_uni)

# Plot the model
dyplot.prophet(m, forecast.apple)
prophet_plot_components(m, forecast.apple)
